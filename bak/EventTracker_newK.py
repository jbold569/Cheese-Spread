...
def cluster(vecs, k):
	global EVENT_ID
	af = AffinityPropagation(preference=None).fit(vecs.values())
	cluster_centers = af.cluster_centers_indices_
	cluster_tweet_pair = zip(vecs.keys(), af.labels_)

	events = []
	for i in range(len(cluster_centers)):
		EVENT_ID += i
		events.append(Event(EVENT_ID, clusters_centers[i]))
		for id, label in cluster_tweet_pair:
			if label == i:
				event[i].addTweet((id,vecs[id]))
	#for event in events:
