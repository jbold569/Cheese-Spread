...
class Server(object):
	...
	def QueryEvents(self, startDate, startTime, endDate, endTime):
		...
		self.event_start_date = dt.datetime(int(date[2]), int(date[1]), int(date[0]), int(time[0]), int(time[1]))
		date = endDate.split('-')
		time = endTime.split(':')
		self.event_end_date = dt.datetime(int(date[2]), int(date[1]), int(date[0]), int(time[0]), int(time[1]))
		# Mongo sequence
		...
		results = DBI.queryTweets(start_time=self.event_start_date, end_time=self.event_end_date)
		...
		data = {}
		data['events'] = []
		for i in range(5*self.event_counter,5*self.event_counter+5):
			data['events'].append(self.events.value()[date_counter][i].toDictionary())
